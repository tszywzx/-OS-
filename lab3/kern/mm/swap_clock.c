...
{     
     ...
     //cprintf("Clock Algorithm Initialized. curr_ptr = %x\n", curr_ptr);
     cprintf("curr_ptr %p\n", curr_ptr);
     return 0;
}
/*
...
_clock_map_swappable(struct mm_struct *mm, uintptr_t addr, struct Page *page, int swap_in)
{
    ...
    page->visited = 1;
    //cprintf("Page %x is added to swap queue. curr_ptr = %x\n", page, curr_ptr);
    cprintf("curr_ptr %p\n", curr_ptr);
    return 0;
}
/*
...
_clock_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
    ...
    while (1) {
        ...
        // 判断页面的访问标志
        if (page->visited == 0) {
            // 页面未被访问，则将其从链表中删除并设置为换出页面
            list_del(curr_ptr);
            *ptr_page = page;
            // 输出换出页面的信息
            //cprintf("Page %x is selected for swap out. curr_ptr = %x\n", *ptr_page, curr_ptr);
            cprintf("curr_ptr %p\n", curr_ptr);
            // 将 curr_ptr 移动到下一个页面
            ...
            // 输出 curr_ptr 的地址值用于调试
            //cprintf("Page %x was accessed, reset visited. curr_ptr = %x\n", page, curr_ptr);
            cprintf("curr_ptr %p\n", curr_ptr);
        }
    }
    return 0;
