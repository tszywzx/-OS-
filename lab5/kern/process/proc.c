...
alloc_proc(void) {
     ...
     */
     ...
     //LAB5 2213034 : (update LAB4 steps)
     /*
...
kernel_execve(const char *name, unsigned char *binary, size_t size) {
 ...
 // 设置ebreak进入中断处理程序处理异常，a7寄存器设置为10表示此处不是简单的中断异常，要调用syscall
    asm volatile(
        "li a0, %1\n"
        "lw a1, %2\n"
        "lw a2, %3\n"
        ...
        "sw a0, %0\n"
        : "=m"(ret)
        : "i"(SYS_exec), "m"(name), "m"(len), "m"(binary), "m"(size) // 执行SYS_exec(name=exit)
        : "memory");
    cprintf("ret = %d\n", ret);
    return ret;
}
